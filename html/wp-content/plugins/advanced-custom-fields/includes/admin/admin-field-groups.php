...
class acf_admin_field_groups {
	...
	*/
	...
	function list_table_views( $views ) {
		...
		// add view
		$views['json'] = '<a' . $class . ' href="' . admin_url($this->url . '&post_status=sync') . '">' . esc_html__('Sync available', 'acf') . ' <span class="count">(' . esc_attr( $total ) . ')</span></a>';
...
<div class="acf-column-2">
	<div class="acf-box">
		<div class="inner">
			...
			<p><?php esc_html_e('Customise WordPress with powerful, professional and intuitive fields.'); ?></p>
			...
			<ul>
				<li><a href="<?php echo esc_url( $url_home ); ?>" target="_blank"><?php echo $icon; ?> <?php esc_html_e("Website",'acf'); ?></a></li>
				<li><a href="<?php echo esc_url( $url_home . '/resources/' ); ?>" target="_blank"><?php echo $icon; ?> <?php esc_html_e("Documentation",'acf'); ?></a></li>
				<li><a href="<?php echo esc_url( $url_support ); ?>" target="_blank"><?php echo $icon; ?> <?php esc_html_e("Support",'acf'); ?></a></li>
				<?php if( !acf_get_setting('pro') ): ?>
				<li><a href="<?php echo esc_url( $url_home . '/pro/' ); ?>" target="_blank"><?php echo $icon; ?> <?php esc_html_e("Pro",'acf'); ?></a></li>
				<?php endif; ?>
			</ul>
		</div>
		<div class="footer">
...
(function($){
	...
	$('#the-list tr').each(function(){
		...
		if( id ) {
			...
			var $span = $('<span class="acf-duplicate-field-group"><a title="<?php esc_attr_e('Duplicate this item', 'acf'); ?>" href="' + esc_attr(url.replace('__post_id__', post_id)) + '"><?php esc_html_e('Duplicate', 'acf'); ?></a> | </span>');
	...
	// modify bulk actions
	$('#bulk-action-selector-bottom option[value="edit"]').attr('value','acfduplicate').text('<?php esc_html_e( 'Duplicate', 'acf' ); ?>');
	...
	// search
	$('.subsubsub').append(' | <li><a href="#" class="acf-toggle-search"><?php esc_html_e('Search', 'acf'); ?></a></li>');
	...
	<tr <?php if($i%2 == 0): ?>class="alternate"<?php endif; ?>>
		...
		<td class="post-title page-title column-title">
			...
			<div class="row-actions">
				<span class="import"><a title="<?php echo esc_attr( __('Synchronise field group', 'acf') ); ?>" href="<?php echo esc_url($url); ?>"><?php esc_html_e( 'Sync', 'acf' ); ?></a></span>
			</div>
		</td>
...
<script type="text/html" id="tmpl-acf-bulk-actions">
	...
	<?php submit_button( esc_html__( 'Apply' ), 'action', '', false, array( 'id' => "doaction2" ) ); ?>
</script>
<script type="text/javascript">
(function($){
	...
	// set only options
	$('#bulk-action-selector-bottom').html('<option value="-1"><?php esc_html_e('Bulk Actions'); ?></option><option value="acfsync"><?php esc_html_e('Sync', 'acf'); ?></option>');
