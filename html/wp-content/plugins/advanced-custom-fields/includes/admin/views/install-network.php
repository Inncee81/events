...
<div id="acf-upgrade-wrap" class="wrap">
	...
	<h1><?php esc_html_e("Advanced Custom Fields Database Upgrade",'acf'); ?></h1>

	<p><?php echo sprintf( esc_html__("The following sites require a DB upgrade. Check the ones you want to update and then click %s.", 'acf'), '"' . $button . '"'); ?></p>

	<p><input type="submit" name="upgrade" value="<?php echo esc_attr($button); ?>" class="button" id="upgrade-sites"></p>
	...
	<table class="wp-list-table widefat">
		...
		<thead>
			<tr>
				...
				<th class="manage-column" scope="col" style="width:33%;"><label for="sites-select-all"><?php esc_html_e("Site", 'acf'); ?></label></th>
		...
		<tr>
			...
			<th class="manage-column" scope="col"><label for="sites-select-all-2"><?php esc_html_e("Site", 'acf'); ?></label></th>
			<th><?php esc_html_e("Description", 'acf'); ?></th>
		</tr>
		</tfoot>
		...
		<?php foreach( $sites as $i => $site ): ?>
			...
			<tr<?php if( $i % 2 == 0 ): ?> class="alternate"<?php endif; ?>>
				...
				<td>
					<strong><?php echo esc_html($site['name']); ?></strong><br /><?php echo esc_html($site['url']); ?>
				</td>
				<td>
				<?php if( $site['updates'] ): ?>
					<span class="response"><?php printf( esc_html__('Site requires database upgrade from %s to %s', 'acf'), $site['acf_version'], $plugin_version); ?></span>
				<?php else: ?>
	...
	<p class="show-on-complete"><?php echo sprintf( esc_html__('Database Upgrade complete. <a href="%s">Return to network dashboard</a>', 'acf'), network_admin_url() ); ?></p>
		...
		var upgrader = {
			...
			upgrade: function(){
				...
				var $input = this.$inputs.eq( this.i ),
					$tr = $input.closest('tr'),
					text = '<?php esc_html_e('Upgrade complete', 'acf'); ?>';
