	...
	*/
	...
	function ajax_move_field() {
		...
		if( $args['field_id'] && $args['field_group_id'] ) {
			...
			// message
			$a = '<a href="' . admin_url("post.php?post={$field_group['ID']}&action=edit") . '" target="_blank">' . esc_html($field_group['title']) . '</a>';
			echo '<p><strong>' . esc_html__('Move Complete.', 'acf') . '</strong></p>';
			echo '<p>' . sprintf( __('The %s field can now be found in the %s field group', 'acf'), $field['label'], $a ). '</p>';
			echo '<a href="#" class="button button-primary acf-close-popup">' . esc_html__("Close Window",'acf') . '</a>';
			die();
		...
		echo '<form id="acf-move-field-form">';
			...
			echo '<button type="submit" class="button button-primary">' . esc_html__("Move Field",'acf') . '</button>';
