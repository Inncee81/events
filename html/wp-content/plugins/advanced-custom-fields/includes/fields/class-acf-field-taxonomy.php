				...
				<?php if( $field['field_type'] == 'radio' && $field['allow_null'] ): ?>
					<li>
						<label class="selectit">
							<input type="radio" name="<?php echo esc_attr($field['name']); ?>" value="" /> <?php esc_html_e("None", 'acf'); ?>
						</label>
					</li>
				<?php endif; ?>
	...
	*/
	...
	function ajax_add_term() {
		...
		if( !current_user_can( $taxonomy_obj->cap->manage_terms) ) {
			...
			echo '<p><strong>' . esc_html__("Error.", 'acf') . '</strong> ' . sprintf( __('User unable to add new %s', 'acf'), $taxonomy_label ) . '</p>';
			die;
		...
		if( $args['term_name'] ) {
			...
			if( term_exists($args['term_name'], $field['taxonomy']) ) {
				...
				wp_send_json_error(array(
					'error'	=> sprintf( esc_html__('%s already exists', 'acf'), $taxonomy_label )
				));
		...
		?><p class="acf-submit"><button class="acf-button button button-primary" type="submit"><?php esc_html_e("Add", 'acf'); ?></button><i class="acf-spinner"></i><span></span></p></form><?php
